...
//  ludik
//
//  Created by Apprenant 123 on 07/06/2024.
//
...
struct JeuxQuizView: View {
   ...
   // - La variable `mots` est un tableau contenant des tuples. Chaque tuple représente un mot avec son image , l'orthographe correcte et une liste d'orthographes incorrectes.
   //- Chaque tuple represente:`(image, orthographe correcte, [orthographes incorrectes])`.
    let mots = [
        ("Hippopotamus", "hippopotame", ["hippopotame", "ippopotame", "hipopotame"]),
        ("Chevaux", "Chevaux", ["chevau", "chevo", "Chevaux"]),
        ("Citrouille", "Citrouille", ["Citrouye", "Citrouille", "Citroulle"]),
    ...
    func checkAnswer(_ index: Int) {
        if gameStarted && mots[currentIndex].2[index] == mots[currentIndex].1 {
            words += 1
        }
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                ForEach (0..<3) { index in
                        Button(
                            ...
                            label: {  Text(mots[currentIndex].2[index])
            ...
            .alert(isPresented: $showCorrectAnswer) {
                Alert(title: Text("Réponse").bold(), message: Text("La réponse correcte est \(mots[currentIndex].1)"), dismissButton: .default(Text("OK")) {
                    nextQuestion()})
            }
